// data.js

// Templates data
export const templates = [
  {
    image: "/images/collections/collection-32.jpg",
    alt: "App Landing",
    items: "20 items",
    name: "Blazers",
  },
  {
    image: "/images/collections/collection-33.jpg",
    alt: "Digital Agency",
    items: "10 items",
    name: "Sweaters",
  },
  {
    image: "/images/collections/collection-34.jpg",
    alt: "Marketing Startup",
    items: "12 items",
    name: "Sneakers",
  },
  {
    image: "/images/collections/collection-35.jpg",
    alt: "Digital Agency",
    items: "21 items",
    name: "Glasses",
  },
  {
    image: "/images/collections/collection-36.jpg",
    alt: "Digital Agency",
    items: "40 items",
    name: "Accessories",
  },
  {
    image: "/images/collections/collection-37.jpg",
    alt: "Marketing Startup",
    items: "25 items",
    name: "Men",
  },
  {
    image: "/images/collections/collection-38.jpg",
    alt: "Marketing Startup",
    items: "35 items",
    name: "Shoes",
  },
];

// Products data
export const products = [
  {
    image: "/images/products/beige-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/light-blue-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/white-12.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/beige-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/beige-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/beige-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/beige-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
  {
    image: "/images/products/beige-2.jpg",
    alt: "Ribbed Tank Top",
    price: "$16.95",
    colors: [
      { bg: "bg-yellow-500" },
      { bg: "bg-black" },
      { border: "border-gray-300", bg: "bg-white" },
    ],
  },
];

// data.js

// Products data grouped by categories
export const categories = [
  {
    name: "Clothing",
    products: [
      {
        image: "/images/products/beige-2.jpg",
        alt: "Ribbed Tank Top",
        price: "$16.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/beige-3.jpg",
        alt: "Ribbed Tank Top",
        price: "$17.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/black-1.jpg",
        alt: "Ribbed Tank Top",
        price: "$18.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/black-14.jpg",
        alt: "Ribbed Tank Top",
        price: "$19.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/black-17.jpg",
        alt: "Ribbed Tank Top",
        price: "$20.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/black-2.jpg",
        alt: "Ribbed Tank Top",
        price: "$21.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/black-3.jpg",
        alt: "Ribbed Tank Top",
        price: "$22.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/products/blue.jpg",
        alt: "Ribbed Tank Top",
        price: "$23.95",
        colors: [
          { bg: "bg-yellow-500" },
          { bg: "bg-black" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
    ],
  },
  {
    name: "Accessories",
    products: [
      {
        image: "/images/shop/gallery/gallery-3.jpg",
        alt: "Gucchi Bag",
        price: "$29.99",
        colors: [
          { bg: "bg-gray-700" },
          { border: "border-gray-300", bg: "bg-white" },
        ],
      },
      {
        image: "/images/shop/gallery/gallery-5.jpg",
        alt: "Hat",
        price: "$30.95",
        colors: [{ bg: "bg-brown-500" }, { bg: "bg-black" }],
      },
      {
        image: "/images/shop/gallery/gallery-6.jpg",
        alt: "Nike Shoes",
        price: "$31.95",
        colors: [{ bg: "bg-brown-500" }, { bg: "bg-black" }],
      },
      {
        image: "/images/shop/gallery/gallery-7.jpg",
        alt: "Rayban Glasses",
        price: "$32.95",
        colors: [{ bg: "bg-brown-500" }, { bg: "bg-black" }],
      },
    ],
  },
  {
    name: "Footwear",
    products: [
      {
        image: "/images/products/dark-blue.jpg",
        alt: "Running Shoes",
        price: "$49.95",
        colors: [{ bg: "bg-red-500" }, { bg: "bg-blue-500" }],
      },
      {
        image: "/images/products/green.jpg",
        alt: "Casual Sneakers",
        price: "$50.95",
        colors: [
          { bg: "bg-green-500" },
          { bg: "bg-white", border: "border-gray-300" },
        ],
      },
      {
        image: "/images/products/light-blue-2.jpg",
        alt: "Casual Sneakers",
        price: "$51.95",
        colors: [
          { bg: "bg-green-500" },
          { bg: "bg-white", border: "border-gray-300" },
        ],
      },
      {
        image: "/images/products/light-green-1.jpg",
        alt: "Casual Sneakers",
        price: "$52.95",
        colors: [
          { bg: "bg-green-500" },
          { bg: "bg-white", border: "border-gray-300" },
        ],
      },
    ],
  },
];
